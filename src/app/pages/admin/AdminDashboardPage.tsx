...
const CATEGORY_LABELS: Record<string, string> = {
  hate_harassment: 'Quấy rối/thù hận',
  sexual: 'Khiêu dâm',
  self_harm: 'Tự hại',
  violence: 'Bạo lực',
  child_safety: 'An toàn trẻ em',
  spam_scam: 'Spam/lừa đảo',
  medical_misinfo: 'Y khoa sai lệch',
  pii: 'Lộ thông tin cá nhân',
};

const AUDIT_ACTION_LABELS: Record<string, string> = {
  content_flagged: 'Nội dung bị đánh dấu',
  hide_post: 'Ẩn bài viết',
  delete_post: 'Xóa bài viết',
  hide_comment: 'Ẩn bình luận',
  delete_comment: 'Xóa bình luận',
  assign_customer: 'Gán khách hàng',
  live_message: 'Tin nhắn chat',
  create_expert: 'Tạo chuyên gia',
  update_expert: 'Cập nhật chuyên gia',
  delete_expert: 'Xóa chuyên gia',
};
...
export function AdminDashboardPage() {
  ...
  const load = useCallback(() => {
    ...
    Promise.all([
      ...
      apiFetch<{ entries: AuditEntry[] }>('/api/admin/audit?limit=50', { token }),
    ])
      .then(([c, e, a, p, r, au]) => {
  ...
  return (
    <div className="space-y-8" style={{ color: tezcaTheme.text }}>
      ...
      <section className="rounded-2xl border p-5 space-y-5" style={tezcaCardStyle}>
              ...
              {pendingReports.map((r) => (
                ...
                >
                  <p className="m-0 mb-1.5">
                    {r.source === 'auto' ? (
                      <span
                        className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold mr-1.5 align-middle"
                    ...
                    {r.source === 'auto' ? 'đánh dấu' : 'báo cáo'}{' '}
                    <span className="font-medium opacity-80">{r.targetType === 'post' ? 'bài viết' : 'bình luận'}</span>
                    {r.reason ? ` · ${r.reason}` : ''}
                  </p>
        ...
        {/* Audit log */}
        <div>
          <h3 className="text-sm font-semibold m-0 mb-2">Nhật ký hoạt động ({auditEntries.length})</h3>
          {auditEntries.length === 0 ? (
            <p className="text-sm opacity-60 m-0 italic">Chưa có hoạt động nào được ghi nhận.</p>
          ) : (
            <ul className="space-y-1.5 list-none m-0 p-0 max-h-72 overflow-y-auto">
              {auditEntries.map((a) => (
                <li
                  key={a.id}
