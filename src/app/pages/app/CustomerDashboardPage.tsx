...
export function CustomerDashboardPage() {
  ...
  return (
    <div
      ...
      style={{ backgroundColor: tezcaTheme.surface, color: tezcaTheme.text }}
    >
          ...
          <section className="min-h-[480px]">
            <div className="rounded-2xl p-6 border flex flex-col h-full relative overflow-hidden" style={tezcaCardStyle}>
              ...
              <div className="flex-1 overflow-y-auto pr-2 space-y-3 relative z-10 tezca-dash-scrollbar min-h-0">
                {isVerifying && exercises.length === 0 ? (
                  <SessionLoading title="Đang tải kế hoạch tập…" minHeight="12rem" hint="" />
                ) : exercises.length === 0 ? (
                  <EmptyState
                    icon={ClipboardList}
                    title="Chưa có bài tập hôm nay"
                    description={
                      trainingStatus === 'pending_review'
                        ? 'Kế hoạch đang chờ chuyên gia duyệt. Bạn vẫn có thể tạo kế hoạch AI tạm thời.'
                        : 'Tạo kế hoạch AI hoặc đợi chuyên gia gán lịch tập cho bạn.'
                    }
                    actionLabel="Tạo kế hoạch"
                    actionTo={ROUTES.app.plans}
                  />
                ) : null}
                {exercises.map((ex) => (
                  <div
