...
/**
 * Đọc audit log cho admin (mới nhất trước). Lọc theo action (tùy chọn).
 * @param {{ action?: string, limit?: number }} opts
 */
export function listAuditLog({ action, limit = 100 } = {}) {
  const lim = Math.min(Math.max(Number(limit) || 100, 1), 500);
  let sql = `
    SELECT a.id, a.ts, a.actor_id AS actorId, a.role, a.action, a.patient_id AS subjectId, a.meta,
           u.name AS actorName
    FROM audit_log a
