...
export function assertProductionSecrets() {
  ...
  if (getJwtSecret() === DEV_JWT) {
    const msg =
      '[security] CRITICAL: Production yêu cầu JWT_SECRET (không dùng giá trị mặc định). Đặt trong Vercel Environment Variables.';
    console.error(msg);
    // Từ chối khởi động — không bao giờ chạy production với secret mặc định.
    throw new Error(msg);
  }
