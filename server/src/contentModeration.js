...
/**
 * Gate kiểm duyệt cho AI chat (input người dùng + output mô hình).
 * Trả { blocked, reply, result }.
 *   - blocked=true  → KHÔNG gọi LLM; dùng `reply` làm câu trả lời an toàn.
 *   - self_harm luôn ưu tiên thông điệp khủng hoảng (115).
 * @param {string} text
 * @returns {{ blocked: boolean, reply: string, result: ReturnType<typeof moderateText> }}
 */
export function moderateChatInput(text) {
  const result = moderateText(text);
  if (result.action !== 'block') {
    return { blocked: false, reply: '', result };
  }
  if (result.categories.includes('self_harm')) {
    return {
      blocked: true,
      result,
      reply:
        'Mình rất lo cho bạn. Nếu bạn đang có ý định tự làm hại mình, hãy gọi ngay 115 hoặc đường dây nóng sức khỏe tâm thần 1800 1567 (miễn phí). Nếu có thể, hãy ở cạnh người bạn tin tưởng. Bạn không một mình, và có người muốn giúp bạn.',
    };
  }
  if (result.categories.includes('child_safety')) {
    return {
      blocked: true,
      result,
      reply: 'Mình không thể hỗ trợ nội dung này. Nếu cần, mình sẵn sàng giúp bạn về dinh dưỡng, vận động và lối sống lành mạnh.',
    };
  }
  return {
    blocked: true,
    result,
    reply:
      'Nội dung này nằm ngoài phạm vi mình có thể hỗ trợ (' +
      result.reasons.join('; ') +
      '). Mình có thể giúp bạn về dinh dưỡng, vận động an toàn, BMI và lối sống lành mạnh.',
  };
}

/**
 * Kiểm duyệt output của mô hình. Chặn khi moderateText quyết định 'block'
 * (đã gồm score>=4 HOẶC category nguy hiểm), TRỪ khi tín hiệu block chỉ đến từ
 * pii/spam_scam đơn lẻ — vì output sức khỏe hợp lệ có thể chứa số liệu/đường
 * dẫn mà không phải nội dung độc hại (tránh chặn nhầm). Trả { safe, reply }.
 * @param {string} text
 */
export function moderateAiOutput(text) {
  const result = moderateText(text);
  if (result.action !== 'block') return { safe: true, reply: text };

  // Các category thực sự nguy hiểm cho output y tế.
  const harmful = result.categories.filter(
    (c) => c !== 'pii' && c !== 'spam_scam',
  );
  // Block chỉ do pii/spam (không có category nguy hiểm) → cho qua, tránh chặn
  // nhầm câu trả lời sức khỏe hợp lệ.
  if (harmful.length === 0) return { safe: true, reply: text };

  return {
    safe: false,
    reply:
      'Xin lỗi, mình không thể đưa ra nội dung này. Mình có thể giúp bạn về dinh dưỡng, vận động an toàn và lối sống lành mạnh.',
  };
}
