...
export function signToken(user, sessionRole) {
  ...
  return jwt.sign(
    { sub: user.id, role, email: user.email, name: user.name, type: 'access' },
    getJwtSecret(),
    { expiresIn: '2h' },
  );
}
...
export function signRefreshToken(user, sessionRole) {
  const role = sessionRole || user.role;
  return jwt.sign(
    { sub: user.id, role, email: user.email, type: 'refresh' },
    getJwtSecret(),
    { expiresIn: '7d' },
  );
}

/** Verify refresh token — chỉ chấp nhận type=refresh */
