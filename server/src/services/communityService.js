...
/**
 * Tạo báo cáo TỰ ĐỘNG cho admin khi nội dung bị đánh dấu (flag) — đóng vòng kiểm duyệt.
 * Gọi SAU khi đã tạo post/comment (cần targetId). Chống trùng theo target.
 * @param {{ result: object, targetType: 'post'|'comment', targetId: string }} arg
 */
function autoReportIfFlagged({ result, targetType, targetId }) {
  if (!result || result.action !== 'flag' || !targetId) return;
  try {
    if (hasPendingAutoReport(targetType, targetId)) return;
    createCommunityReport({
      id: randomUUID(),
      targetType,
      targetId,
      reporterId: null,
      reason: moderationMessage(result) || result.reasons.join('; '),
      source: 'auto',
      categories: result.categories.join(','),
    });
  } catch {
    /* không chặn luồng chính nếu tạo báo cáo lỗi */
  }
}
...
export function addThreadReply(input) {
  const mod = guardContent({ userId: input.userId, kind: 'thread_reply', text: input.content });
  const post = createCommunityThreadReply(input);
  if (post) {
    autoReportIfFlagged({ result: mod, targetType: 'post', targetId: post.id });
...
export function createPost(input) {
  const mod = guardContent({ userId: input.userId, kind: 'post', text: input.content });
...
export function addComment(input) {
  const mod = guardContent({ userId: input.userId, kind: 'comment', text: input.content });
  const comment = createCommunityComment(input);
  if (comment) {
    autoReportIfFlagged({ result: mod, targetType: 'comment', targetId: comment.id });
