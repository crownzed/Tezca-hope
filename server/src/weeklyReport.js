import {
  ...
  listBmiForUserInRange,
  listMoodsForUserInRange,
  listBotMessagesForUserInRange,
  listLiveMessagesForCustomerInRange,
} from './db.js';
...
export function buildWeeklyReport(expertId, weekStartIso) {
  ...
  const customers = customerIds.map((cid) => {
    ...
    const live = listLiveMessagesForCustomerInRange(cid, startTs, endTs);
    const bmi = listBmiForUserInRange(cid, from, to);
    const moods = listMoodsForUserInRange(cid, from, to);
    const bot = listBotMessagesForUserInRange(cid, startTs, endTs);
