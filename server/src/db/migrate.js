...
export function runMigrations(db) {
  ...
  const migrations = [
    ...
    {
      version: 27,
      name: 'community_reports_auto_source',
      up: () => {
        // Cho phép báo cáo TỰ ĐỘNG từ bộ lọc nội dung: reporter_id NULL, thêm source + categories.
        // SQLite không ALTER được CHECK/NOT NULL → rebuild bảng giữ dữ liệu cũ.
        const hasSource = tableHasColumn(db, 'community_reports', 'source');
        if (hasSource) return;
        db.exec(`
        ...
        `);
        db.exec(`
          INSERT INTO community_reports_new
            (id, target_type, target_id, reporter_id, reason, source, categories, status, created_at)
          SELECT id, target_type, target_id, reporter_id, reason, 'user', '', status, created_at
          FROM community_reports;
        `);
        db.exec(`DROP TABLE community_reports;`);
        db.exec(`ALTER TABLE community_reports_new RENAME TO community_reports;`);
        db.exec(`CREATE INDEX IF NOT EXISTS idx_community_reports_status ON community_reports(status, created_at DESC);`);
      },
    },
  ];
