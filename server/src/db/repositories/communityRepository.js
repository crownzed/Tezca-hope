...
export function createCommunityReport({ id, targetType, targetId, reporterId = null, reason = '', source = 'user', categories = '' }) {
  getDb()
    .prepare(
      `INSERT INTO community_reports (id, target_type, target_id, reporter_id, reason, source, categories, status, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?)`,
    )
    .run(id, targetType, targetId, reporterId, reason, source, categories, Date.now());
  return { id, targetType, targetId, source, status: 'pending' };
}

/**
 * Chống trùng: đã có báo cáo TỰ ĐỘNG đang chờ cho cùng target chưa?
 * Tránh tạo nhiều auto-report khi user sửa bài nhiều lần.
 */
export function hasPendingAutoReport(targetType, targetId) {
  const row = getDb()
    .prepare(
      `SELECT id FROM community_reports
       WHERE target_type = ? AND target_id = ? AND source = 'auto' AND status = 'pending'
       LIMIT 1`,
    )
...
export function listCommunityReports({ status } = {}) {
  ...
  return getDb()
    ...
    .map((r) => ({
      ...
      reporterName: r.reporter_name || (r.source === 'auto' ? 'Bộ lọc tự động' : null),
      reason: r.reason,
