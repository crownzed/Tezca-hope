...
function asyncRoute(handler) {
  return (req, res) => {
    Promise.resolve(handler(req, res)).catch((err) => {
      ...
      if (!res.headersSent) {
        res.status(500).json({
          error: err instanceof Error ? err.message : 'Lỗi máy chủ cộng đồng',
        });
      }
    });
  };
...
communityRouter.get(
  ...
  requireMember,
  asyncRoute(async (req, res) => {
    const userIds = await communityService.listFollowedUserIds(req.user.sub);
    res.json({ userIds });
  }),
);
...
communityRouter.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    await communityService.followUser(req.user.sub, followingId);
    res.json({ ok: true });
  }),
);
...
communityRouter.delete(
  ...
  asyncRoute(async (req, res) => {
    const followingId = String(req.params.userId);
    await communityService.unfollowUser(req.user.sub, followingId);
    res.json({ ok: true });
  }),
);
...
communityRouter.get(
  ...
  requireMember,
  asyncRoute(async (req, res) => {
    const topics = await communityService.listFollowedTopics(req.user.sub);
    res.json({ topics });
  }),
);
...
communityRouter.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    await communityService.followTopic(req.user.sub, topic);
    res.json({ ok: true });
  }),
);
...
communityRouter.delete(
  ...
  asyncRoute(async (req, res) => {
    ...
    await communityService.unfollowTopic(req.user.sub, topic);
    res.json({ ok: true });
  }),
);
...
communityRouter.get(
  ...
  requireMember,
  asyncRoute(async (req, res) => {
    const post = await communityService.getPost(String(req.params.postId), req.user.sub);
    if (!post) {
...
communityRouter.delete(
  ...
  asyncRoute(async (req, res) => {
    ...
    const ok = await communityService.removeOwnPost(
      postId,
      req.user.sub,
...
communityRouter.get(
  ...
  asyncRoute(async (req, res) => {
    const postId = String(req.params.postId);
    const post = await communityService.getPost(postId, req.user.sub);
    if (!post) {
      res.status(404).json({ error: 'Không tìm thấy bài viết' });
      return;
    }
...
communityRouter.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    const replies = await communityService.listThreadReplies(postId, { viewerId: req.user.sub });
    res.json({ replies });
  }),
);
...
communityRouter.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const post = await communityService.addThreadReply({
      id: crypto.randomUUID(),
      userId: req.user.sub,
      parentPostId: postId,
      content,
...
communityRouter.post(
  ...
  asyncRoute(async (req, res) => {
    const postId = String(req.params.postId);
    const result = await communityService.likePost(postId, req.user.sub);
    if (!result) {
      res.status(404).json({ error: 'Không tìm thấy bài viết' });
      return;
    }
...
communityRouter.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    const messages = await realtimeChatService.listRoomMessages(topic, {
...
communityRouter.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const message = await realtimeChatService.sendRoomMessage({
      id: crypto.randomUUID(),
      topic,
      userId: req.user.sub,
      content,
...
communityRouter.get(
  '/rooms/:topic/mention-candidates',
  requireMember,
  asyncRoute(async (req, res) => {
    const topic = String(req.params.topic);
    if (!isValidRoomTopic(topic)) {
    ...
    const members = await communityExtendedService.mentionCandidates(topic, { query: q });
    res.json({ members });
  }),
);
...
communityRouter.get(
  ...
  asyncRoute(async (req, res) => {
    const since = req.query.since ? Number(req.query.since) : undefined;
    const messages = await communityExtendedService.listAnnouncements({
      sinceTs: Number.isFinite(since) ? since : undefined,
    });
    res.json({ messages });
  }),
...
communityRouter.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const message = await communityExtendedService.postAnnouncement({
      id: crypto.randomUUID(),
      userId: req.user.sub,
      content,
    });
...
communityRouter.get(
  ...
  requireMember,
  asyncRoute(async (req, res) => {
    const threads = await communityExtendedService.listDmThreads(req.user.sub);
    res.json({ threads });
  }),
);
...
communityRouter.get(
  ...
  asyncRoute(async (req, res) => {
    const q = String(req.query.q || '').trim();
    const members = await communityExtendedService.searchMembers({
      query: q,
...
communityRouter.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const thread = await communityExtendedService.openDmThread(req.user.sub, otherUserId);
    if (!thread) {
...
communityRouter.get(
  ...
  requireMember,
  asyncRoute(async (req, res) => {
    const thread = await communityExtendedService.getDmThread(String(req.params.threadId), req.user.sub);
    if (!thread) {
...
communityRouter.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    const since = req.query.since ? Number(req.query.since) : undefined;
    const messages = await communityExtendedService.listDmMessages(threadId, req.user.sub, {
      sinceTs: Number.isFinite(since) ? since : undefined,
    });
    if (!messages) {
      res.status(404).json({ error: 'Không tìm thấy cuộc trò chuyện' });
...
communityRouter.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const message = await communityExtendedService.sendDmMessage({
      id: crypto.randomUUID(),
      threadId,
