...
import { signToken, signRefreshToken, verifyRefreshToken, resolveDbUser } from '../auth.js';
...
function loginHandler(requiredRole) {
  return (req, res) => {
    ...
    const refreshToken = signRefreshToken(user, sessionRole);
    res.json({
      token,
      refreshToken,
      user: {
...
authRouter.post('/refresh', (req, res) => {
  ...
  if (!dbUser) {
    res.status(401).json({ error: 'Tài khoản không còn hợp lệ' });
    return;
  }
  const token = signToken(dbUser, payload.role);
  res.json({ token });
});
